 ...
 */
export default async function SeriesPage({ params, searchParams }: PageProps<"/[locale]/series">) {
  ...
  const initialPage = await catalogService
    .listSeries({ ...toListParams(filters), page: 1 }, toLocale((await params).locale))
    .catch(() => null);

  return (
    <PageSection className="py-14 pb-24">
      <div className="flex flex-col gap-10">
        <PageHeading overline={t.catalog.overline} title={t.catalog.seriesTitle} />
        {/* `CatalogBrowser` reads the URL, so it needs a Suspense boundary. */}
        <Suspense fallback={<TitleGridSkeleton />}>
          <CatalogBrowser kind="series" initialPage={initialPage} />
        </Suspense>
      </div>
    </PageSection>
  );
